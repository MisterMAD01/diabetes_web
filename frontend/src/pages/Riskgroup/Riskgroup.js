...
const RiskGroup = () => {
  ...
  const colorGroups = [
    { name: "สีขาว", bg: "#ffffff", text: "#000000", popup: WhitePopup },
    {
      ...
      bg: "#72d572",
      text: "#000000",
      popup: LightGreenPopup,
    },
    {
      name: "สีเขียวเข้ม",
      bg: "#056f00",
      text: "#ffffff",
      popup: DarkGreenPopup,
    },
  ...
  const openPopup = async (colorName) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API}/api/risk/list/${colorName}`
      );
  ...
  return (
    <div className="form-container">
      <h2 className="color-group-title">
        กลุ่มผู้ป่วยจำแนกตามเกณฑ์ปิงปองจราจร 7 สี
      </h2>
